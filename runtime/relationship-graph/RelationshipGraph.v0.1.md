...
在新的“数字我”架构里，`RelationshipGraph` 属于表我层：它描述别人眼里的我、我和不同人的社会关系、亲密度、共同经历、称呼、场景和权限边界。

这里的关键前提是：不同人眼里的我不是同一个外显版本。稳定的本我在 `SelfCore` 里，但表我会随对象变化：

- 亲密度不同，表达密度和玩笑强度不同。
- 共同经历不同，可默认引用的背景不同。
- 关系边界不同，能主动聊的话题不同。
- 历史样本量不同，数字我对这个人的表达置信度不同。
- 未映射或低亲密度群友只能使用低亲密度表达，不能假装熟悉。
...
这一部分不应理解为“用户的通用说话风格”。它是该人物眼里的我，即 `SelfCore` 在这个关系里的外显表现型。后续高置信度实现应优先使用 `DyadicProfile(person_id)`，而不是关系大类模板。
...
权限也属于表我的一部分：同一句话、同一个主动动作，对不同关系的风险不同。直接微信关系、已授权群友、被提及人物和陌生人必须分开处理。
